...
*/
const newFruit = [ 'orange', 'apple', 'banana', 'apples', 'apple', 'mango' ]
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inven, num) {
  return `This is a ${inven[num-1].car_make} ${inven[num-1].car_model}`
}
// console.log(getCarInfoById(data, 7))
/**
...
*/
function getOlderCars(inven, maxYear) {
  oldInven= []
  for (i=0; i<inven.length; i++){
    if (inven[i].car_year <= maxYear)
    oldInven.push (inven[i])
  }
  return oldInven
}
// console.log(getOlderCars(data, 2005))
 ...
 * in the same order as they appear in the original inventory.
*/
function getGermanCars(inven) {
  germCars = []
  for(i=0;i<inven.length;i++){
    if (inven[i].car_make === 'Audi' || inven[i].car_make === 'Mercedes-Benz' || inven[i].car_make === 'Volkswagen' || inven[i].car_make === 'BMW'){
      germCars.push(inven[i]);
      }   
  }
  return germCars;
}
